...
This is a design document, not a commitment. Each stage stands alone and
can be shipped or abandoned independently of the next. Stage 1 is the only
one I'd recommend committing to today; later stages are written up here so
that if they are revisited, the homework is already done.
...
|---|---|---|---|---|
| **Stage 1** — Modern Player Colors | Unlock 6→8 bit per channel; per-worm `ColorBlock` indirection; one curated alternate palette ("Modern Vivid"); `ColorMode` enum on Renderer; netplay protocol bump for 24-bit worm color | 1–2 days | None | Yes |
...
## Stage 1 — Modern Player Colors

### Goal

Give players a richer worm-color picker without changing the game's overall
look or asset pipeline. Set up the architectural prep that makes Stage 2,
hot-toggle, and per-window mode cheap if/when they're built.

### Scope

In:

- Remove the `& 63` clamp at `palette.cpp:63-65` (palette load) and revisit
  `Palette::ScaleAdd` and `Palette::SetWormColoursSpan` to operate on 8-bit
  per channel (preserve current visual output in classic mode by feeding
  6-bit-equivalent input from the legacy palette file).
- Widen `WormSettings.rgb` to a `uint8_t[3]` in 0..255 (currently
  effectively 0..63). Update `settings.cpp` (default values, TOML
  serialization at lines 73, 83, 105, 137, 162).
- Replace the magic constants `kWormSpriteColorBase[2] = {32, 41}` and
  `kWormColourIndexes[2] = {0x58, 0x78}` and the literals `30-34` / `39-43`
  with a per-worm `ColorBlock { int base; int width; RampParams ramp; }`.
  Default values preserve current behavior. The indirection unlocks future
  wider ramps and custom skin sprites at minimal additional cost.
- Bump the netplay protocol version. Audit `WormSettings` serialization
  on the wire (search `worm_settings` and `WormSettings::rgb` in
  `src/game/net/`); widen the field to 24 bits.
- Add a `ColorMode { Classic, Modern }` enum on `Renderer`. The per-frame
  palette rebuild becomes:
  ```cpp
  renderer.pal = (renderer.mode == Modern) ? renderer.origpal_modern
                                           : renderer.origpal_classic;
  ```
  `origpal_classic` is the existing palette loaded from the EXE/TC.
  `origpal_modern` is the new "Modern Vivid" palette shipped as a data
  file in `data/`.
- Ship one alternate palette file ("Modern Vivid"). Format identical to
  classic `pal` files for simplicity; the modern path just doesn't apply
  the 6-bit clamp on load.

Out (deferred to later stages or never):

- Any change to sprite assets, level data, or the simulation.
- True-color sprites, true-color UI, alpha blending.
- True-color terrain.
- Multiple curated palettes beyond Modern Vivid (could be added later as
  pure data).

### Cost & risk

~1-2 days. Determinism unaffected (renderer is downstream of sim).
Netplay protocol bump is mechanical (no installed base concerns since
we're pre-1.0). Replay format unaffected (palette state isn't snapshotted
into replays; replays render through whatever palette the current build
uses).

### Verification

- [ ] Loading a classic-VGA-era palette and rendering a worm in classic
      mode produces bit-identical output to the current build.
- [ ] Picker accepts and round-trips 0..255 RGB through TOML.
- [ ] Picking a vivid color (e.g., neon orange `255,128,32`) and switching
      the renderer to Modern mode produces visibly richer worm shading.
- [ ] Net protocol bump: a Modern client and a Classic-only old client
      either negotiate gracefully or refuse to connect with a clear error.
- [ ] Existing replays play back unchanged.

### Why this is the right v1

It's the smallest change that delivers visible user value, and it pays
exactly the right architectural prep to make every subsequent stage
cheaper:

- `ColorMode` on `Renderer` is the spine of the hot-toggle and per-window
  features.
- `ColorBlock` indirection is the spine of any future custom-skin work.
- Per-renderer `origpal_classic` / `origpal_modern` is the spine of every
  later "pick a different palette" feature without touching the rest of
  the engine.
...
A keypress (F11) — or a settings-menu entry, or a per-game choice in the
new-game flow — swaps `Renderer.mode` live. Next frame uses the new mode.

**Stage 1:** essentially free. The per-frame palette rebuild
(`gfx.cpp:861-874`) already exists; we just pick `origpal_classic` or
`origpal_modern` based on `renderer.mode`. **+0.5 days** for the key binding
and the conditional.
...
**Stage 1:** move the global "color mode" preference into a per-renderer
`mode` field. Each renderer's per-frame palette rebuild reads
`renderer.mode`. **+0.5 days** on top of the hot-toggle work (mostly
shared).
...
**Rejected as a standalone v1**, but **adopted as part of Stage 1** (we
do ship Modern Vivid as a curated palette, alongside the per-channel
unlock).

The reason not to do D-only as the entire v1: the user explicitly wanted
the picker to feel meaningfully more capable, not just "same VGA picker
but with three preset color schemes." Combining D with the 6→8 bit
unlock gives the toggle something concrete to switch between (immediate
visible difference) AND gives the picker meaningfully more expressive
range.
...
- **Wire protocol bump mechanics.** Where exactly is `WormSettings.rgb`
  serialized in the netplay handshake? (`src/game/net/transport.cpp:349,
  523` look relevant.) Is there a clean version field, or do we need to
  introduce one? Pre-1.0 status suggests this is a small concern.
- **TOML format for the worm color.** Today it's stored as three integers
  in 0..63 range. Just widen the range; the TOML parser doesn't care. But
  worth documenting the change so old config files load with reasonable
  defaults.
...
- **Loading community-contributed palettes from disk.** Initially out;
  the engine ships with the two curated palettes (Classic, Modern Vivid).
  If demand exists, loading custom palettes is a data-level change later.
...
**Recommended first commitment:** Stage 1 only. ~1-2 days of work, ships
the visible "modern player colors" feature, pays the forward-looking
architectural cost (`ColorMode` on `Renderer`, `ColorBlock` indirection,
per-renderer `origpal_classic` / `origpal_modern`) that makes everything
else cheap if/when it's wanted, and commits to nothing further.

Stages 2 and 3 and the cross-cutting features can be revisited when
player or TC-author demand makes their cost worth paying. The homework
in this document is intended to make that revisit cheap.
