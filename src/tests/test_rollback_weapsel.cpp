...
// Regression for the ENet UNRELIABLE_SEQUENCED bug that broke remote
// netplay in the WS phase.
//
// The bug, end to end:
//   - Host transitions to Playing and starts emitting input batches
//     before the ICE bridge in the client's direction is fully open.
//   - ENet eventually delivers them out of order; on a SEQUENCED
//     channel, batches with an "old" sequence number are dropped if a
//     "newer" one was processed first.
//   - The client's first received batch is base=2+ — host's frame 1
//     input is gone forever.
//   - Client's confirm chain sticks at frame 0 (prefilled), runs
//     predicted up to kMaxRollback, then the rollback-window stall
//     fires. The host stalls symmetrically via the frame-advantage
//     guard once it's far enough ahead of the now-frozen client.
//
// The fix sends batches with ENET_PACKET_FLAG_UNSEQUENCED so older
// batches arriving after newer ones still reach the application. The
// receive-side dedup in injectRemoteInput against confirmedSimFrame_
// makes the redundant-overlap a cheap no-op.
//
// This test models the wire-level distinction in-process via a
// per-direction "highest delivered localFrame" counter. Toggling
// sequencedDrop on/off shows the controller stalls under the bug and
// recovers under the fix, given the same delivery jitter.
TEST_CASE("WS phase stalls under sequenced-drop, recovers under unsequenced",
          "[rollback][weapsel][regression]") {
  constexpr uint32_t kWorldSeed = 0xFEEDFACE;

  // Holds batches sent in a tick and replays them on a later tick.
  // Models a warm-up gap where the channel is open one-way before it
  // is open the other way, so the first N batches queue and end up
  // sequenced behind newer batches that get out first.
  struct Held {
    uint8_t gen;
    uint32_t baseFrame;
    uint8_t count;
  ...
  auto runScenario = [&](bool sequencedDrop, int32_t& outAFrame,
                         int32_t& outBFrame, int32_t& outAConf,
                         int32_t& outBConf) {
    auto [common, settings] = makeEnv();
    auto a = makePeer(common, settings, 0, kWorldSeed);
    auto b = makePeer(common, settings, 1, kWorldSeed);

    std::vector<Held> aOut, bOut;
    int tick = 0;

    auto cap = [&](std::vector<Held>& q) {
      return [&q, &tick](uint8_t gen, uint32_t bf, uint8_t c,
                         uint8_t const* in, uint32_t lf) {
        Held h{};
        h.gen = gen; h.baseFrame = bf; h.count = c; h.localFrame = lf;
        h.sentTick = tick;
        for (uint8_t i = 0; i < c; ++i) h.inputs[i] = in[i];
        q.push_back(h);
      };
    };
    a->setInputCallbacks(cap(aOut));
    b->setInputCallbacks(cap(bOut));
    a->focus();
    b->focus();
    a->injectRemoteInput(0, 0);
    b->injectRemoteInput(0, 0);

    // For first kWarmupTicks ticks, batches "queue" and are not yet
    // visible. Starting tick kWarmupTicks, they all become eligible
    // for delivery — but the catch-up tick delivers freshest-first
    // (reverse send order), so on a sequenced channel the older queued
    // batches are dropped. From kWarmupTicks+1 onward, delivery is in
    // send order at zero delay.
    constexpr int kWarmupTicks = 6;
    int32_t aToBHighest = -1;
    int32_t bToAHighest = -1;

    auto flushQueue = [&](std::vector<Held>& q,
                          int32_t& highest,
                          RollbackController& dst,
                          bool reverse) {
      if (reverse) {
        for (auto it = q.rbegin(); it != q.rend(); ++it) {
          if (sequencedDrop && static_cast<int32_t>(it->localFrame) < highest)
            continue;  // sequenced channel discards this older batch
          highest = static_cast<int32_t>(it->localFrame);
          dst.injectRemoteBatch(it->gen, it->baseFrame, it->count,
                                it->inputs.data(), it->localFrame);
        }
      } else {
        for (auto& h : q) {
          if (sequencedDrop && static_cast<int32_t>(h.localFrame) < highest)
            continue;
          highest = static_cast<int32_t>(h.localFrame);
          dst.injectRemoteBatch(h.gen, h.baseFrame, h.count, h.inputs.data(),
                                h.localFrame);
        }
      }
      q.clear();
    ...
    for (tick = 0; tick < kTotalTicks; ++tick) {
      a->setLocalControlState(0);
      b->setLocalControlState(0);
      a->process();
      b->process();
      ...
      if (tick < kWarmupTicks) {
        // Queue — bridge not open for delivery yet.
        continue;
      }
      if (tick == kWarmupTicks) {
        // Catch-up burst: deliver everything queued, in reverse send
        // order (mirrors ENet processing a backlog freshest-first).
        flushQueue(aOut, aToBHighest, *b, /*reverse=*/true);
        flushQueue(bOut, bToAHighest, *a, /*reverse=*/true);
      } else {
        // Steady state: deliver in send order.
        flushQueue(aOut, aToBHighest, *b, /*reverse=*/false);
        flushQueue(bOut, bToAHighest, *a, /*reverse=*/false);
      }
    }

    outAFrame = static_cast<int32_t>(a->currentFrame());
    outBFrame = static_cast<int32_t>(b->currentFrame());
    outAConf = a->confirmedFrame();
    outBConf = b->confirmedFrame();
  };

  int32_t buggyA, buggyB, buggyAConf, buggyBConf;
  runScenario(/*sequencedDrop=*/true, buggyA, buggyB, buggyAConf, buggyBConf);

  int32_t fixedA, fixedB, fixedAConf, fixedBConf;
  runScenario(/*sequencedDrop=*/false, fixedA, fixedB, fixedAConf, fixedBConf);

  INFO("buggy:  A simFrame=" << buggyA  << " confirmed=" << buggyAConf
       << " | B simFrame=" << buggyB  << " confirmed=" << buggyBConf);
  INFO("fixed:  A simFrame=" << fixedA  << " confirmed=" << fixedAConf
       << " | B simFrame=" << fixedB  << " confirmed=" << fixedBConf);

  // Bug demonstration: at least one peer is in an unhealthy state —
  // its confirm chain trails its simFrame by more than the rollback
  // window (rollback-window stall), or its simFrame itself stalled
  // short of where steady-state delivery would have taken it.
  bool buggyHealthy =
      (buggyA - buggyAConf) <= rollback::kMaxRollback &&
      (buggyB - buggyBConf) <= rollback::kMaxRollback &&
      buggyA > rollback::kMaxRollback * 3 &&
      buggyB > rollback::kMaxRollback * 3;
  REQUIRE_FALSE(buggyHealthy);

  // Fix demonstration: with the same delivery jitter but no sequenced
  // discard, both peers' confirm chains keep up with their simFrames
  // (within the rollback window) and the WS phase makes real progress
  // well past the warm-up.
  REQUIRE(fixedA > rollback::kMaxRollback * 3);
  REQUIRE(fixedB > rollback::kMaxRollback * 3);
  REQUIRE((fixedA - fixedAConf) <= rollback::kMaxRollback);
  REQUIRE((fixedB - fixedBConf) <= rollback::kMaxRollback);
}
