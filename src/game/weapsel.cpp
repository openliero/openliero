...
WeaponSelection::WeaponSelection(Game& game)
  ...
  for (std::size_t i = 0; i < menus.size(); ++i) {
    ...
    bool randomWeapons =
        (ws.controller != 0 && game.settings->selectBotWeapons == 0);

    for (int j = 0; j < Settings::selectableWeapons; ++j) {
      if (ws.weapons[j] == 0 || randomWeapons) {
        ws.weapons[j] = std::uniform_int_distribution<int>(1, 40)(gfx.rand);
      }
...
void WeaponSelection::drawSpectatorViewports(
    Renderer& renderer,
    GameState state) {
  Common& common = *game.common;
  ...
  if (!cachedSpectatorBackground) {
    if (game.settings->levelFile.empty()) {
      common.font.drawCenteredText(
          renderer.bmp, LS(LevelRandom), centerX, centerY - 32, 7, 2);
    } else {
      auto levelName = getBasename(getLeaf(gfx.settings->levelFile));
      common.font.drawCenteredText(
          renderer.bmp, LS(LevelIs1) + levelName + LS(LevelIs2), centerX,
          centerY - 32, 7, 2);
    }

    std::string vsText = game.settings->wormSettings[0]->name + " vs " +
                         game.settings->wormSettings[1]->name;
    ...
    fillRect(
        renderer.bmp, centerX - (textSize / 2) - 1, centerY + 23 - 1, 16, 16,
        7);
    fillRect(
        renderer.bmp, centerX - textSize / 2, centerY + 23, 14, 14,
        game.settings->wormSettings[0]->color);
    fillRect(
        renderer.bmp, centerX + (textSize / 2) - 16 - 1, centerY + 23 - 1, 16,
        16, 7);
    fillRect(
        renderer.bmp, centerX + textSize / 2 - 16, centerY + 23, 14, 14,
        game.settings->wormSettings[1]->color);
    common.font.drawCenteredText(
        renderer.bmp, "WEAPON SELECTION", centerX, centerY + 48, 7, 2);
    game.level.drawMiniature(
        renderer.bmp, centerX - 126, renderer.renderResY - 208, 2);
  ...
  // TODO: This just uses the currently activated palette, which might well be
  // wrong.
  gfx.singleScreenRenderer.pal = gfx.singleScreenRenderer.origpal;
  gfx.singleScreenRenderer.pal.rotateFrom(
      gfx.singleScreenRenderer.origpal, 168, 174, gfx.menuCycles);
...
void WeaponSelection::drawNormalViewports(Renderer& renderer, GameState state) {
  ...
  if (!cachedBackground) {
    ...
    if (game.settings->levelFile.empty()) {
      ...
      common.font.drawText(
          renderer.bmp, (LS(LevelIs1) + levelName + LS(LevelIs2)), 0, 162, 50);
    }
  ...
  for (std::size_t i = 0; i < menus.size(); ++i) {
    ...
    drawRoundedBox(
        renderer.bmp, weaponMenu.x + 29 - width / 2, weaponMenu.y - 11, 0, 7,
        width);
    common.font.drawText(
        renderer.bmp, ws.name, weaponMenu.x + 31 - width / 2, weaponMenu.y - 10,
        ws.color + 1);
  ...
  gfx.playRenderer.pal.rotateFrom(
      gfx.playRenderer.origpal, 168, 174, gfx.menuCycles);
          ...
          do {
            ...
            if (ws.weapons[weapID] < 1)
              ws.weapons[weapID] = (uint32_t)common.weapons.size();
                       ...
                       ->weapTable[common.weapOrder[ws.weapons[weapID] - 1]] !=
                   0);
      ...
      if (worm.pressed(Worm::Fire)) {
        if (menus[i].selection() == 0) {
          ...
          for (int j = 0; j < Settings::selectableWeapons; ++j) {
            while (true) {
              ws.weapons[j] =
                  std::uniform_int_distribution<int>(1, 40)(gfx.rand);
