  ...
  if (worm.killedTimer <= 0) {
    if (worm.visible) {
      if (worm.steerableCount > 0) {
        setCenter(
            worm.steerableSumX / worm.steerableCount,
...
void Viewport::draw(
  ...
  if (worm.visible) {
    ...
    drawBar(
        renderer.bmp, worm.statsX * multiplier, renderer.renderResY - 39,
  ...
  if (ww.available()) {
    if (ww.ammo > 0) {
      ...
      if (ammoBarWidth > 0)
        drawBar(
            renderer.bmp, worm.statsX * multiplier, renderer.renderResY - 34,
    ...
    if ((game.cycles % 20) > 10 && worm.visible) {
      common.font.drawText(
          renderer.bmp, LS(Reloading), worm.statsX * multiplier,
          164 * multiplier, 50);
    }
  }
  ...
  common.font.drawText(
      ...
      worm.statsX * multiplier, renderer.renderResY - 29, 10);

  if (isReplay) {
    common.font.drawText(
        renderer.bmp, worm.settings->name, worm.statsX * multiplier,
        renderer.renderResY - 15, 7);
    fillRect(
        renderer.bmp, worm.statsX * multiplier, renderer.renderResY - 7 - 1, 8,
        8, 7);
    fillRect(
        renderer.bmp, (worm.statsX + 1) * multiplier, renderer.renderResY - 7,
        6, 6, worm.settings->color);
    common.font.drawText(
  ...
  switch (game.settings->gameMode) {
    ...
    case Settings::GameMode::Holdazone: {
      ...
      common.font.drawText(
          renderer.bmp, timeToString(worm.timer), 5 * multiplier,
...

  {
    ...
    if (!worm.visible && worm.killedTimer <= 0 && !worm.ready) {
      common.font.drawText(
          renderer.bmp, LS(PressFire), rect.center_x() - 30, 76, 0);
      common.font.drawText(
    ...
    for (std::size_t i = 0; i < game.viewports.size(); ++i) {
      ...
      if (v != this && otherWorm.health <= 0 && v->bannerY > -8) {
        if (otherWorm.lastKilledByIdx == worm.index) {
          ...
          common.font.drawText(
              renderer.bmp, msg, rect.x1 + 3, v->bannerY + 1, 0);
    ...
    for (std::size_t i = 0; i < game.worms.size(); ++i) {
      ...
      if (w.visible) {
        ...
        if (w.ninjarope.out) {
          ...
          drawNinjarope(
              common, renderer.bmp, ninjaropeX, ninjaropeY, tempX + 7,
              tempY + 4);
        ...
        if (game.settings->shadow)
          blitShadowImage(
              common, renderer.bmp,
              common.wormSprite(w.currentFrame, w.direction, w.index),
