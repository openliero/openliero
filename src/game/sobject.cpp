...
void SObjectType::create(
  ...
  if (startSound >= 0)
    game.soundPlayer->play(
        std::uniform_int_distribution<int>(0, numSounds - 1)(game.rand) +
        startSound);

  for (std::size_t i = 0; i < game.viewports.size(); ++i) {
    Viewport& v = *game.viewports[i];

    if (x > v.x && x < v.x + v.rect.width() && y > v.y &&
        y < v.y + v.rect.height()) {
  ...
  if (damage > 0) {
    for (std::size_t i = 0; i < game.worms.size(); ++i) {
      ...
      if (wix < x + detectRange && wix > x - detectRange &&
          wiy < y + detectRange && wiy > y - detectRange) {
        int delta = wix - x;
          ...
          if (bloodAmount > 0) {
            for (int i = 0; i < bloodAmount; ++i) {
              ...
              common.nobjectTypes[6].create2(
                  game, angle, w.vel / 3, w.pos, 0, w.index, firedBy);
            }
          }
    ...
    for (WObject* i; (i = wr.next());) {
      ...
      if (weapon.affectByExplosions) {
        ...
        if (ipos.x < x + detectRange && ipos.x > x - detectRange &&
            ipos.y < y + detectRange && ipos.y > y - detectRange) {
          int delta = ipos.x - x;
          int power = detectRange - std::abs(delta);
        ...
        for (int x = rect.x1; x < rect.x2; ++x) {
          if (game.level.mat(x, y).anyDirt() &&
            ...
            common.nobjectTypes[2].create2(
                game, angle, fixedvec(), itof(gvl::ivec2(x, y)), pix, ownerIdx,
                firedBy);
          }
        }
    }
  ...
  if (dirtEffect >= 0) {
    ...
    if (game.settings->shadow)
      correctShadow(
          common, game.level, gvl::rect(x - 10, y - 10, x + 11, y + 11));
  }
