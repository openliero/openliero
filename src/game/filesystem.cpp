...
#define BOOST_SYSTEM_DIRECTORY_TYPE struct dirent*
  ...
  inline filename_result find_first_file(
      const char* dir,
      BOOST_HANDLE& handle,
...
#define BOOST_SYSTEM_DIRECTORY_TYPE WIN32_FIND_DATAA
  ...
  // Returns: 0 if error, otherwise name
  {
    ...
    bool fail =
        ((handle = ::FindFirstFileA(dirpath.c_str(), &data)) ==
...
void DirectoryListing::sort() {
  std::sort(subs.begin(), subs.end(), [](NodeName const& a, NodeName const& b) {
...
std::shared_ptr<FsNodeImp> join(
    std::shared_ptr<FsNodeImp> a,
    std::shared_ptr<FsNodeImp> b);
...
struct FsNodeZipFile : FsNodeImp {
  ...
  FsNodeZipFile(std::string const& path, bool isDir)
    ...
    for (mz_uint fileIndex = 0; fileIndex < fileCount; ++fileIndex) {
      ...
      bool isDir =
          mz_zip_reader_is_file_a_directory(&archive->archive, fileIndex);
  ...
  gvl::source tryToSource() override {
    ...
    std::size_t size;
    auto* ptr = mz_zip_reader_extract_to_heap(
        &archive->archive, (mz_uint)fileIndex, &size, 0);
...
struct FsNodeFilesystem : FsNodeImp {
  ...
  std::shared_ptr<FsNodeImp> go(std::string const& name) override {
    ...
    if (access(zipPath.c_str(), 0) != -1) {
      ...
      imp = join(
          std::move(imp),
          std::shared_ptr<FsNodeImp>(new FsNodeZipFile(zipPath, true)));
    } else if (
        access(fullPath.c_str(), 0) != -1 && endsWith(fullPath, ".zip")) {
      imp = join(
          std::move(imp),
          std::shared_ptr<FsNodeImp>(new FsNodeZipFile(fullPath, true)));
    }
