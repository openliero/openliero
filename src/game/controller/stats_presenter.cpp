...
std::string percent(int nom, int den) {
  ...
  char buf[BUF_MAX];
  std::snprintf(
      buf, BUF_MAX * sizeof(char), "%.2f%%", double(nom) * 100.0 / den);
  return buf;
}
...
struct StatsRenderer {
  ...
  void drawWorms() {
    hblock(20, [this] {
      for (int i = 0; i < 2; ++i) {
        int x = renderer.renderResX / 2 +
                (i == 0 ? -1 : 1) * (renderer.renderResX / 4) + offsX;
        blitImage(
            renderer.bmp, common.wormSpriteObj(2, i == 0 ? 1 : 0, i), x - 8, y);

        cell c(i == 0 ? cell::right : cell::left);
        common.font.drawText(
            renderer.bmp, c << game.worms[i]->settings->name,
  ...
  void drawWorm(int i) {
    ...
    if (!visible) {
      int x = 18 + offsX;
      blitImage(
          renderer.bmp, common.wormSpriteObj(2, i == 0 ? 1 : 0, i), x - 8, 10);
    }
  }
  ...
  void drawWormStat(char const* name, Ws wormStat) {
    hblock(11, [this, name, &wormStat] {
      common.font.drawText(
          renderer.bmp, cell(cell::center).ref() << name,
  ...
  void drawStat(char const* name, Stat stat) {
    hblock(11, [this, name, &stat] {
      common.font.drawText(
          renderer.bmp, cell(cell::right).ref() << name,
          renderer.renderResX / 2 + offsX, y, textColor);
  ...
  void graph(
    ...
    hblock(height, [&, this] {
      int start = 20 + offsX;
      drawGraph(
          renderer.bmp, data, height, start, y, color, negColor, balanced);
    });
    y += 7;
  }
...
void sortWeaponStats(vector<WeaponStats>& ws) {
  std::sort(
      ws.begin(), ws.end(), [](WeaponStats const& a, WeaponStats const& b) {
        return a.actualHp > b.actualHp;
      });
}
...
void presentStats(NormalStatsRecorder& recorder, Game& game) {
  ...
  for (int i = 0; i < 2; ++i) {
    wormDamages[i] = stretch(
        convert<double>(
            pluck(recorder.worms[i].wormFrameStats, &WormFrameStats::damage)),
        graphWidth);
    cumulative(wormDamages[i]);
    normalize(wormDamages[i], 50);

    wormTotalHp[i] = convert<double>(
        pluck(recorder.worms[i].wormFrameStats, &WormFrameStats::totalHp));
  }

  vector<double> wormTotalHpDiff(stretch(
      zip(wormTotalHp[0], wormTotalHp[1], std::minus<double>()), graphWidth));
  ...
  while (true) {
    ...
    renderer.pane(0, offsX, offsY, [&] {
      ...
      renderer.graph(
          wormTotalHpDiff, 100, Palette::wormColourIndexes[0],
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      renderer.pane(i == 0 ? -1 : 1, offsX, offsY, [&] {
        ...
        renderer.graph(
            wormDamages[i], 50, Palette::wormColourIndexes[i], 0, false);
    ...
    gfx.playRenderer.pal = common.exepal;  // We don't use gfx.origpal because
                                           // the colors are unpredictable
    ...
    if (gfx.testSDLKey(SDL_SCANCODE_DOWN)) {
      ...
      destPane = std::max(destPane - 1.0, -1.0);
        ...
        gfx.testSDLKey(SDL_SCANCODE_RETURN) ||
