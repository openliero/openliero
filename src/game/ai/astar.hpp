...
struct path_node : gvl::pairing_node<path_node> {
  ...
  path_node() : parent(0), state(none) {}

  bool operator<(path_node const& b) const { return f < b.f; }
...
struct astar_state {
  ...
  // open_list contains elements of nodes, so it's important that open_list is
  // last
  gvl::pairing_heap<path_node_t, std::less<path_node_t>, gvl::dummy_delete>
      open_list;
  ...
  bool a_star(
      NodeT origin,
      NodeT target,
      SuccessorIter succ_iter,
      ResultOutputIterator result,
      Heuristic heuristic) {
    ...
    while (!state.open_list.empty()) {
      ...
      // from the open_list in the next iteration. Is this useful? I suppose
      // not.
...
struct level_cell_succ {
  ...
  operator bool() const { return i < 8; }

  int node() {}

  int cost() { return edges[i].cost; }
