        ...
        for (std::size_t offs = skip; offs < items.size(); ++offs) {
          ...
          if (items[i].visible && menuString.size() >= newPrefix.size()) {
            ...
            if (contains) {
              result =
                  std::search(
                      menuString.begin(), menuString.end(), newPrefix.begin(),
              ...
              result = std::equal(
                  newPrefix.begin(), newPrefix.end(), menuString.begin(),
                  [](char a, char b) {
                    return std::toupper((unsigned char)a) ==
                           std::toupper((unsigned char)b);
                  });
...
void Menu::draw(
  ...
  for (int c = itemFromVisibleIndex(topItem);
    ...
    item.draw(
        common, renderer, x, curY, selected, disabled, centered, valueOffsetX);
