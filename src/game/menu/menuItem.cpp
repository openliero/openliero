...
void MenuItem::draw(
  ...
  if (selected) {
    ...
    if (hasValue)
      drawRoundedBox(
          renderer.bmp, x + valueOffsetX - (valueWid >> 1), y, 0, 7, valueWid);
  } else {
    common.font.drawText(renderer.bmp, string, x + 3, y + 2, 0);
    if (hasValue)
      common.font.drawText(
          renderer.bmp, value, x + valueOffsetX - (valueWid >> 1) + 3, y + 2,
          0);
  }
  ...
  if (hasValue)
    common.font.drawText(
        renderer.bmp, value, x + valueOffsetX - (valueWid >> 1) + 2, y + 1, c);
}
