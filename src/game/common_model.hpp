...
#define NObj(n)      \
  ar.ref(            \
      #n, nobject.n, \
      ObjectResolver<NObjectType>(common, common.nobjectTypes));
#define SObj(n)      \
  ar.ref(            \
      #n, nobject.n, \
      ObjectResolver<SObjectType>(common, common.sobjectTypes));
    B(wormExplode)
    B(explGround)
    B(wormDestroy)
    B(drawOnMap)
    ...
    I(numFrames)
    I(colorBullets)
    SObj(createOnExp) I(dirtEffect) I(splinterAmount) I(splinterColour)
        NObj(splinterType) I(bloodTrailDelay) SObj(leaveObj) I(leaveObjDelay)
            I(timeToExplo) I(timeToExploV)
...
#define NObj(n) \
  ar.ref(       \
      #n, weapon.n, ObjectResolver<NObjectType>(common, common.nobjectTypes));
#define SObj(n) \
  ar.ref(       \
      #n, weapon.n, ObjectResolver<SObjectType>(common, common.sobjectTypes));
    S(name)
    ...
    I(splinterAmount)
    I(splinterColour)
    NObj(splinterType) I(splinterScatter) SObj(objTrailType) I(objTrailDelay)
        I(partTrailType) NObj(partTrailObj) I(partTrailDelay) SObj(createOnExp)
            B(chainExplosion)
...
void archive_text(Common& common, Archive& ar) {
  ar.obj("types", [&] {
    ar.arr("sounds", common.sounds, [&](SfxSample& s) { ar.str(0, s.name); });

    ar.arr("weapons", common.weapons, [&](Weapon& w) { ar.str(0, w.idStr); });
