...
namespace gvl {
  ...
  namespace toml {
    ...
    struct writer {
      ...
      writer(Writer& w) : indent(0), inObject(false), w(w) {}
    ...
    enum type { t_null, t_bool, t_integer, t_string, t_object, t_array };
    ...
    struct value {
      ...
      value(int v) : tt(t_integer) { u.i = v; }

      value(bool v) : tt(t_bool) { u.i = v; }

      value(value const& other) : tt(other.tt), u(other.u) {
        if (tt >= t_string)
          u.s->add_ref();
      }

      value(value&& other) : tt(other.tt), u(other.u) { other.tt = t_null; }
    ...
    inline value::value(object&& o) : tt(t_object) {
      u.s = new object(o);
    }

    inline value::value(array&& a) : tt(t_array) {
      u.s = new array(a);
    }

    inline value::value(string&& s) : tt(t_string) {
      u.s = new string(s);
    }
    ...
    struct reader {
      ...
      reader(Reader& r) : root(object()), r(r) { start(); }
