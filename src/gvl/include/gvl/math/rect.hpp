...
namespace gvl {
  ...
  class basic_rect {
   public:
    basic_rect() : x1(0), y1(0), x2(0), y2(0) {}

    basic_rect(T x1_, T y1_, T x2_, T y2_)
        : x1(x1_), y1(y1_), x2(x2_), y2(y2_) {}

    basic_rect(basic_vec<T, 2> const& pixel)
        : x1(pixel.x), y1(pixel.y), x2(pixel.x + T(1)), y2(pixel.y + T(1)) {}
    ...
    basic_rect flip() const { return basic_rect<T>(y1, x1, y2, x2); }
    ...
    bool inside(T vx, T vy) const {
      ...
      return diffX < width() && diffX >= T(0) && diffY < height() &&
             diffY >= T(0);
    }
    ...
    bool proper_intersecting(basic_rect const& b) const {
      return (b.y2 > y1 && b.y1 < y2 && b.x2 > x1 && b.x1 < x2);
    }
    ...
    bool encloses(basic_vec<T, 2> v) const { return encloses(v.x, v.y); }
    ...
    basic_rect translated(T x, T y) { return *this + basic_vec<T, 2>(x, y); }
    ...
    basic_vec<T, 2> ul() { return basic_vec<T, 2>(x1, y1); }

    basic_vec<T, 2> ur() { return basic_vec<T, 2>(x2, y1); }

    basic_vec<T, 2> ll() { return basic_vec<T, 2>(x1, y2); }

    basic_vec<T, 2> lr() { return basic_vec<T, 2>(x2, y2); }

    bool operator==(basic_rect const& b) const {
      return x1 == b.x1 && y1 == b.y1 && x2 == b.x2 && y2 == b.y2;
    }

    bool operator!=(basic_rect const& b) const { return !operator==(b); }
  };
