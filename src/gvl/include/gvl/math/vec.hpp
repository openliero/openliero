...
namespace gvl {
  ...
  struct basic_vec<T, 2> {
    ...
    basic_vec(T const& x_, T const& y_) : x(x_), y(y_) {}
    ...
    basic_vec half() const { return basic_vec(x / T(2), y / T(2)); }
  ...
  template <typename T>
  inline basic_vec<T, 2> operator-(
      basic_vec<T, 2> const& lhs,
      basic_vec<T, 2> const& rhs) {
    basic_vec<T, 2> ret(lhs);
    ret -= rhs;
    return ret;
  }
  ...
  inline basic_vec<T, 2> cross(T a, basic_vec<T, 2> b) {
    basic_vec<T, 2> ret(-a * b.y, a * b.x);
    return ret;
  }

  template <typename T>
  inline basic_vec<T, 2> cross(basic_vec<T, 2> a, T b) {
    basic_vec<T, 2> ret(b * a.y, -b * a.x);
    return ret;
  }
  ...
  inline basic_vec<T, 2> perp(basic_vec<T, 2> self) {
    basic_vec<T, 2> ret(-self.y, self.x);
    return ret;
  }
