...
namespace gvl {
  ...
  inline void gvl_list_link_after(
      gvl_list_node* self,
      gvl_list_node* new_node) {
  ...
  inline void gvl_list_link_before(
      gvl_list_node* self,
      gvl_list_node* new_node) {
  ...
  struct list_common {
    ...
    gvl_list_node* first() const {
      return static_cast<gvl_list_node*>(sentinel_.next);
    }
    gvl_list_node* last() const {
      return static_cast<gvl_list_node*>(sentinel_.prev);
    }
  ...
  struct list : list_common, protected Deleter, protected Ownership {
    ...
    struct iterator {
      ...
      iterator(gvl_list_node* ptr) : ptr_(ptr) {}

      iterator(T* ptr) : ptr_(upcast(ptr)) {}

      T& operator*() { return *downcast(ptr_); }
      ...
      operator T*() { return downcast(ptr_); }
    ...
    struct reverse_iterator {
      ...
      reverse_iterator(gvl_list_node* ptr) : ptr_(ptr) {}

      reverse_iterator(T* ptr) : ptr_(upcast(ptr)) {}
      ...
      reverse_iterator next() const { return reverse_iterator(ptr_->prev); }

      reverse_iterator prev() const { return reverse_iterator(ptr_->next); }

      bool operator==(reverse_iterator const& b) { return b.ptr_ == ptr_; }
    ...
    struct range {
      range(gvl_list_node* front_init, gvl_list_node* end_init)
          : front_(front_init), end_(end_init) {}

      bool empty() { return front_ == end_; }

      void pop_front() { front_ = front_->next; }

      T& front() { return *downcast(front_); }
    ...
    list(Ownership const& ownership = Ownership()) : Ownership(ownership) {}
    ...
    iterator begin() { return iterator(list_common::first()); }
    ...
    iterator relink(iterator b, T* el) {
      return iterator(list_common::relink(b.ptr_, upcast(el)));
    }

    reverse_iterator relink(reverse_iterator b, T* el) {
      return reverse_iterator(list_common::relink_after(b.ptr_, upcast(el)));
    }
