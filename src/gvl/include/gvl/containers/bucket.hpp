...
namespace gvl {
  ...
  struct bucket_data : shared {
    ...
    void* operator new(std::size_t n) { return new char[n]; }

    void operator delete(void* m) { delete[] static_cast<char*>(m); }
  };
  ...
  struct bucket_data_mem : bucket_data {
    ...
    static bucket_data_mem*
    create_from(uint8_t const* b, uint8_t const* e, std::size_t cap_init) {
      std::size_t s = e - b;
      sassert(cap_init >= s);
  ...
  struct bucket : list_node<> {
    ...
    bucket() : begin_(0), end_(-1) {}

    bucket(bucket const& b, std::size_t begin, std::size_t end)
        : data_(b.data_), begin_(begin), end_(end) {}

    bucket(bucket_data_mem* data) : data_(data), begin_(0), end_(0) {
      end_ = data_->size();
