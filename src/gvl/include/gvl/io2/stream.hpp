...
namespace gvl {
  ...
  struct source_result {
    ...
    source_result(source_result&& other)
        : s(other.s), data(std::move(other.data)) {}

    source_result(status s) : s(s) {}

    source_result(unique_ptr<bucket_data_mem> data)
        : s(ok), data(std::move(data)) {}
  ...
  struct sink_result {
    enum status { ok = 0, part, would_block, error };

    sink_result(status s) : s(s) {}

    operator bool() { return s == ok; }
  ...
  struct stream_piece : shared {
    ...
    stream_piece(shared_ptr<stream_piece> const& next) : next(next) {}

    stream_piece(shared_ptr<bucket_pipe> const& source) : source(source) {}

    stream_piece(shared_ptr<bucket_data_mem> data) : data(std::move(data)) {}
  ...
  struct stream_error : std::runtime_error {
    stream_error(std::string const& msg) : std::runtime_error(msg) {}
  };
  ...
  struct octet_reader {
    ...
    octet_reader(source head) : cur_(0), end_(0), head_(std::move(head)) {}

    octet_reader(shared_ptr<bucket_data_mem> data) : cur_(0), end_(0), head_() {
      set_bucket_(std::move(data));
    }

    octet_reader() : cur_(0), end_(0), head_() {}
    ...
    bool empty() { return cur_ == end_ && !head_; }
  ...
  struct octet_writer : basic_text_writer<octet_writer> {
    ...
    octet_writer(shared_ptr<bucket_pipe> sink) : sink_(), cur_(0), end_(0) {
      attach(sink);
    }

    octet_writer() : sink_(), cur_(0), end_(0) {}
    ...
    sink_result put(uint8_t b) {
      ...
      return (cur_ != end_) ? (*cur_++ = b, sink_result(sink_result::ok))
                            : overflow_put_(b);
    }
    ...
    sink_result put(uint8_t const* p, std::size_t len) {
      ...
      if (left() >= len) {
        ...
        {
          while (len > 4) {
            *reinterpret_cast<uint32_t*>(cur_) =
                *reinterpret_cast<uint32_t const*>(p);
            len -= 4;
            cur_ += 4;
            p += 4;
          }
    ...
    void attach(shared_ptr<bucket_pipe> new_sink) {
      ...
      sink_ = new_sink;
      buffer_.reset(bucket_data_mem::create(
          default_initial_bucket_size, default_initial_bucket_size));
      read_in_buffer_();
    }
   ...
   private:
    ...
    std::size_t buffer_size_() { return (cur_ - buffer_->data); }

    std::size_t left() const { return end_ - cur_; }

    void correct_buffer_() { buffer_->size_ = buffer_size_(); }
