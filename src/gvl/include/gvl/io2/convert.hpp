...
namespace gvl {
  ...
  struct cell : basic_text_writer<cell> {
    ...
    cell() : text_placement(left), width(-1) {}

    cell(placement text_placement_init)
        : text_placement(text_placement_init), width(-1) {}

    cell(int width_init, placement text_placement_init)
        : text_placement(text_placement_init), width(width_init) {}

    void put(uint8_t x) { buffer.push_back(x); }
