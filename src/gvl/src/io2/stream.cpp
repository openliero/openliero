...
namespace gvl {
  ...
  sink_result octet_writer::flush_buffer(bucket_size new_buffer_size) {
    ...
    if (size > 0) {
      ...
      if (res.s != sink_result::ok && res.s != sink_result::would_block)
        return res;
    }

    new_buffer_size =
        std::max(new_buffer_size, std::size_t(default_initial_bucket_size));
  ...
  sink_result octet_writer::overflow_put_(uint8_t const* p, std::size_t len) {
    ...
    while (len > left()) {
      ...
      // Flush and try to allocate a buffer large enough for the rest of the
      // data
