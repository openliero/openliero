...
int CSint16desc[][2] = {
    ...
    {CBonusSpawnRectW,
     0x2311},  // This is used even when the hack isn't enabled
...
int hBonusReloadOnlyInd[][2] = {
    {0x2DB1, 0xEB},  // We check one byte only, because ProMode has a silly jump
                     // destination

    {-1, 0}};

int hBonusSpawnRectInd[][2] = {
    {0x2318, 0x05},  // These are the first bytes of the add instructions that
                     // offset the spawn
    {0x2323, 0x05},
...
void loadWeapons(Common& common, ReaderFile& exe) {
  ...
  readMembers<ReadBool>(
      exe, common.nobjectTypes, &NObjectType::affectByExplosions);
  ...
  readMembers<Dec<Read8> >(
      exe, common.nobjectTypes, &NObjectType::splinterType);
...
void loadGfx(Common& common, ReaderFile& exe, ReaderFile& gfx) {
  ...
  for (int y = 0; y < 16; ++y)
    for (int x = 0; x < 16; ++x) {
      int idx = y * 16 + x;
      common.largeSprites.spritePtr(73)[idx] =
          std::uniform_int_distribution<int>(0, 4 - 1)(rand) + 160;
      common.largeSprites.spritePtr(74)[idx] =
          std::uniform_int_distribution<int>(0, 4 - 1)(rand) + 160;

      common.largeSprites.spritePtr(87)[idx] =
          std::uniform_int_distribution<int>(0, 4 - 1)(rand) + 12;
      common.largeSprites.spritePtr(88)[idx] =
          std::uniform_int_distribution<int>(0, 4 - 1)(rand) + 12;

      common.largeSprites.spritePtr(82)[idx] =
          std::uniform_int_distribution<int>(0, 4 - 1)(rand) + 94;
      common.largeSprites.spritePtr(83)[idx] =
          std::uniform_int_distribution<int>(0, 4 - 1)(rand) + 94;
    }
}
