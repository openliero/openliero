...
struct OctetTextWriter : gvl::octet_writer {
  OctetTextWriter(std::string const& path)
      : gvl::octet_writer(
            gvl::sink(new gvl::file_bucket_pipe(path.c_str(), "wb"))),
        w(*this) {}
...
void writeSpriteTga(gvl::octet_writer& w, SpriteSet& ss, Palette& pal) {
...
void commonSave(Common& common, std::string const& path) {
  ...
  for (auto& s : common.sounds) {
    ...
    gvl::octet_writer w(gvl::sink(new gvl::file_bucket_pipe(
        joinPath(dir, s.name + ".wav").c_str(), "wb")));
...

  {
    ...
    {
      gvl::octet_writer w(gvl::sink(
          new gvl::file_bucket_pipe(joinPath(dir, "small.tga").c_str(), "wb")));
      writeSpriteTga(w, common.smallSprites, common.exepal);
    }

    {
      gvl::octet_writer w(gvl::sink(
          new gvl::file_bucket_pipe(joinPath(dir, "large.tga").c_str(), "wb")));
      writeSpriteTga(w, common.largeSprites, common.exepal);
    }

    {
      gvl::octet_writer w(gvl::sink(
          new gvl::file_bucket_pipe(joinPath(dir, "text.tga").c_str(), "wb")));
      writeSpriteTga(w, common.textSprites, common.exepal);
    }

    {
      gvl::octet_writer w(gvl::sink(
          new gvl::file_bucket_pipe(joinPath(dir, "font.tga").c_str(), "wb")));
